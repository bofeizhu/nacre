//! Ports `graphiti_core/prompts/extract_nodes.py` (pinned v0.29.2).
//!
//! Prompt text is byte-verbatim; interpolation reproduces the Python
//! f-string semantics (`py_interp`) and `to_prompt_json`. Fidelity is
//! enforced against fixtures rendered from the actual upstream Python —
//! see `tests/prompt_fidelity.rs`.
...
// ports: graphiti_core/prompts/extract_nodes.py::extract_message
pub fn extract_message(context: &Value) -> Vec<Message> {
    let sys_prompt = "You are an entity extraction specialist for conversational messages. \
...
"#,
        entity_types = py_interp(&context["entity_types"]),
...
// ports: graphiti_core/prompts/extract_nodes.py::extract_json
pub fn extract_json(context: &Value) -> Vec<Message> {
    let sys_prompt = "You are an entity extraction specialist for JSON data. \
...
"#,
        entity_types = py_interp(&context["entity_types"]),
        source_description = py_interp(&context["source_description"]),
        episode_content = py_interp(&context["episode_content"]),
        custom_extraction_instructions = py_interp(&context["custom_extraction_instructions"]),
    );
    vec![msg(Role::System, sys_prompt), msg(Role::User, user_prompt)]
...
// ports: graphiti_core/prompts/extract_nodes.py::extract_text
pub fn extract_text(context: &Value) -> Vec<Message> {
    let sys_prompt = "You are an entity extraction specialist for unstructured text. \
NEVER extract abstract concepts, feelings, or generic words.";
...
// ports: graphiti_core/prompts/extract_nodes.py::classify_nodes
pub fn classify_nodes(context: &Value) -> Vec<Message> {
    let sys_prompt = "You are an entity classification specialist. \
...
"#,
        previous_episodes = to_prompt_json(&context["previous_episodes"]),
        episode_content = py_interp(&context["episode_content"]),
        extracted_entities = py_interp(&context["extracted_entities"]),
        entity_types = py_interp(&context["entity_types"]),
    );
    vec![msg(Role::System, sys_prompt), msg(Role::User, user_prompt)]
}

// ports: graphiti_core/prompts/extract_nodes.py::extract_attributes
pub fn extract_attributes(context: &Value) -> Vec<Message> {
    let sys_prompt = "You are an entity attribute extraction specialist. \
...
"#,
        ...
        node = py_interp(&context["node"]),
    );
    vec![msg(Role::System, sys_prompt), msg(Role::User, user_prompt)]
}

// ports: graphiti_core/prompts/extract_nodes.py::extract_summary
pub fn extract_summary(context: &Value) -> Vec<Message> {
    let sys_prompt =
...
// ports: graphiti_core/prompts/extract_nodes.py::_entity_type_descriptions_section
fn entity_type_descriptions_section(context: &Value) -> String {
    let descriptions = context.get("entity_type_descriptions");
    let Some(descriptions) = descriptions else {
        return String::new();
    };
    // Python truthiness: None and empty containers produce no section.
    let falsy = match descriptions {
        Value::Null => true,
...
"#,
        descriptions = to_prompt_json(descriptions),
    )
}

// ports: graphiti_core/prompts/extract_nodes.py::extract_summaries_batch
pub fn extract_summaries_batch(context: &Value) -> Vec<Message> {
    let sys_prompt = "You are a helpful assistant that generates concise entity summaries from provided context.";
...
"#,
        ...
        previous_episodes = to_prompt_json(&context["previous_episodes"]),
        episode_content = to_prompt_json(&context["episode_content"]),
        entity_type_descriptions_section = entity_type_descriptions_section(context),
        entities = to_prompt_json(&context["entities"]),
    );
    vec![msg(Role::System, sys_prompt), msg(Role::User, user_prompt)]
}

// NOTE (upstream): this prompt is semantically mirrored in a Go async summary
// worker on Zep's side; upstream keeps both in sync.
// ports: graphiti_core/prompts/extract_nodes.py::_entity_episode_summary_system_prompt
const ENTITY_EPISODE_SUMMARY_SYSTEM_PROMPT: &str = r#"You maintain detailed, information-dense entity memories from episode text.
...
// ports: graphiti_core/prompts/extract_nodes.py::extract_entity_summaries_from_episodes
pub fn extract_entity_summaries_from_episodes(context: &Value) -> Vec<Message> {
    let user_prompt = format!(
        r#"NEVER include meta-language about the summarization process. Use ONLY facts from the provided EPISODES.
...
"#,
    ...
    vec![
        msg(Role::System, ENTITY_EPISODE_SUMMARY_SYSTEM_PROMPT),
        msg(Role::User, user_prompt),
    ]
}
